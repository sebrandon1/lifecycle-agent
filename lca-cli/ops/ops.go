...
const maxConcurrentContainerStops = 10
...
func (o *ops) waitForEtcd(ctx context.Context, healthzEndpoint string) error {
	...
	for {
		select {
		case <-ctx.Done():
			return fmt.Errorf("context cancelled waiting for etcd: %w", ctx.Err())
		case <-timeout:
...
func (o *ops) removeImage(ctx context.Context, img string) error {
	...
	if _, err := o.hostCommandsExecutor.Execute(ctx,
		podman, "rmi", img,
	); err != nil {
...
func (o *ops) RecertFullFlow(ctx context.Context, recertContainerImage, authFile, configFile string,
	...
	defer func() { _ = o.StopEtcdServer(context.WithoutCancel(ctx), authFile, common.EtcdContainerName) }()
...
func (o *ops) getExtraPartitionPath(ctx context.Context, installationDisk string, partitionNumber uint) (string, error) {
	lsblkOutput, err := o.RunInHostNamespace(ctx, "lsblk", installationDisk, "--json", "--output", "NAME,PATH")
	if err != nil {
...
func (o *ops) calculateStartFromNegative(ctx context.Context, installationDisk, negativeStart string) (string, error) {
	...
	diskSizeBytesStr, err := o.RunBashInHostNamespace(ctx, "blockdev", "--getsize64", installationDisk)
	if err != nil {
	...
	sectorSizeStr, err := o.RunBashInHostNamespace(ctx, "blockdev", "--getss", installationDisk)
	if err != nil {
...
func (o *ops) CreateExtraPartition(ctx context.Context, installationDisk, extraPartitionLabel, extraPartitionStart string, extraPartitionNumber uint) error {
	...
	if strings.HasPrefix(extraPartitionStart, "-") {
		calculatedStart, err := o.calculateStartFromNegative(ctx, installationDisk, extraPartitionStart)
		if err != nil {
	...
	extraPartitionPath, err := o.getExtraPartitionPath(ctx, installationDisk, extraPartitionNumber)
	if err != nil {
...
func (o *ops) SetupContainersFolderCommands(ctx context.Context) error {
	...
	if _, err := o.RunInHostNamespace(ctx, "chattr", "-i", "/mnt/"); err != nil {
		return fmt.Errorf("failed to remove immutable flag on /mnt/: %w", err)
	}
	if err := o.MkdirAll("/mnt/containers", 0o755); err != nil {
		// Best-effort: re-seal /mnt/ so it stays immutable even though mkdir failed
		if _, chattrErr := o.RunInHostNamespace(ctx, "chattr", "+i", "/mnt/"); chattrErr != nil {
...
func (o *ops) StopClusterServices(ctx context.Context) error {
	...
	if crioSystemdStatus == "active" {
		...
		_ = wait.PollUntilContextCancel(ctx, time.Second, true, func(pollCtx context.Context) (done bool, err error) {
			o.log.Info("Stop running containers")
			output, listErr := o.RunInHostNamespace(pollCtx, "crictl", "ps", "-q")
			if listErr != nil {
			...
			sem := make(chan struct{}, maxConcurrentContainerStops)
			for _, id := range ids {
				sem <- struct{}{}
				go func(containerID string) {
					defer func() { <-sem }()
					if _, stopErr := o.RunInHostNamespace(pollCtx, "crictl", "stop", "--timeout", "5", containerID); stopErr != nil {
			...
			var firstErr error
			for range ids {
				if stopErr := <-errCh; stopErr != nil && firstErr == nil {
					firstErr = stopErr
				}
			}
			if firstErr != nil {
